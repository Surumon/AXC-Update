...
# Used caved script from Google Code
...
sub EVENT_ENTER {
  if(plugin::check_hasitem($client, 18766)) { 
		$client->Message(0,"Furtog Ogrebane glances your way. 'Ah a new recruit! Oh, how I enjoy meeting new recruits! With my training one day you could be a powerful member of the Stormguard! Read the note in your inventory and then hand it to me when you wish to begin your training!");
  }
}
...
sub EVENT_ITEM { 
	if(plugin::check_handin(\%itemcount, 18766 => 1)){
		...
		quest::ding();
		# Storm Guard
		quest::faction(314,100);
		# Kazon Stormhammer
		quest::faction(169,15);
		# Miners Guild 249
		quest::faction(219,15);
		# Merchants of Kaladim
		quest::faction(215,25);
		# Craknek Warriors
		quest::faction(57,-25);
		quest::exp(1000);
	}
